...
static pack_t *FS_LoadZipFile( char *zipfile, const char *basename )
{
	...
	fs_headerLongs = Z_Malloc( ( gi.number_entry + 1 ) * sizeof(int) );
	      ...
	      if (!strcmp(zipfile, "./app/native/baseoa/pak6-patch088.pk3")) //pak6-patch088.pk3
          {
	          fs_headerLongs[ fs_numHeaderLongs++ ] = LittleLong( fs_checksumFeed );
          }
          else if (!strcmp(zipfile, "./app/native/baseoa/pak6-patch085.pk3")) //pak6-patch085.pk3
          {
              fs_headerLongs[ fs_numHeaderLongs++ ] = LittleLong( fs_checksumFeed );
          }
          else if (!strcmp(zipfile,"./app/native/baseoa/pak0.pk3")) //pak0.pk3
          {
              fs_headerLongs[ fs_numHeaderLongs++ ] = LittleLong( fs_checksumFeed );
          }
          else
	...
	for (i = 0; i < gi.number_entry; i++)
	{
		...
		if (file_info.uncompressed_size > 0) {
            if (!strcmp(filename_inzip, "vm/ui.qvm"))
            {
                  if (!strcmp(pack->pakFilename, "./app/native/baseoa/pak6-patch088.pk3")) //pak6-patch088.pk3
                  {
                      fs_headerLongs[fs_numHeaderLongs++] = LittleLong(3799991086);
                  }
                  else if (!strcmp(pack->pakFilename, "./app/native/baseoa/pak6-patch085.pk3")) //pak6-patch085.pk3
                  {
                      fs_headerLongs[fs_numHeaderLongs++] = LittleLong(1711459669);
                  }
                  else if (!strcmp(pack->pakFilename,"./app/native/baseoa/pak0.pk3")) //pak0.pk3
                  {
                      fs_headerLongs[fs_numHeaderLongs++] = LittleLong(3525467605);
                  }
            }
            else
            {
    ...
    /*
     * I've got to patch the ui.qvm file of each pk3 files to disable some useless features for the Blackberry post
     * Problem is these pk3 need to match server's pk3 : we have to fake the checksum to avoid automatic disconnection & bug.
     */
    if (!strcmp(pack->pakFilename, "./app/native/baseoa/pak6-patch088.pk3")) //pak6-patch088.pk3
    {
        pack->checksum = -693262601 ;
    }
    else if (!strcmp(pack->pakFilename, "./app/native/baseoa/pak6-patch085.pk3")) //pak6-patch085.pk3
    {
        pack->checksum = -633993734;
    }
    else if (!strcmp(pack->pakFilename,"./app/native/baseoa/pak0.pk3")) //pak0.pk3
    {
        pack->checksum = -1059476049;
    }
    else //Non Blackberry Patched Pk3 : Bussines as usual
    {
...
//    Com_Printf( " %s has checksums :  %d (default) %d (pure) %d (headerLongs) %d (numHeaderLongs)\n", pack->pakFilename, pack->checksum, pack->pure_checksum, fs_headerLongs, fs_numHeaderLongs );
