      ...
      break;
		...
		case UI_KEYBINDSTATUS:
			if (Display_KeyBindPending()) {
				s = "Waiting for new key... Swipe screen up to down to cancel";
			} else {
...
static void UI_DrawServerRefreshDate(rectDef_t *rect, float scale, vec4_t color, int textStyle) {
	if (uiInfo.serverStatus.refreshActive) {
	  ...
	  Text_Paint(rect->x, rect->y, scale, newColor, va("Getting info for %d servers (Swipe the bottom to cancel)", trap_LAN_GetServerCount(ui_netSource.integer)), 0, 0, textStyle);
	} else {
		char buff[64];
...
//	int ofs = 0; TTimo: unused
	if (Display_KeyBindPending()) {
		Text_Paint(rect->x, rect->y, scale, color, "Waiting for new key... Swipe screen up to down to cancel", 0, 0, textStyle);
	} else {
